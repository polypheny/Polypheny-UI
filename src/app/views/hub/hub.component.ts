...
export class HubComponent implements OnInit, OnDestroy {
  ...
  changePassword () {
    ...
    if( this.changePwForm.valid ) {
      console.log(this.changePwForm.value['newPw1']);
  ...
  getDatasets () {
    this._hub.getDatasets().subscribe(
      res => {
        ...
        console.log('Could not retrieve datasets: ' + err);
      });
  }
  ...
  newDsFormSubmit(){
    ...
    if(this.newDsForm.valid){
      ...
      myReader.onloadend = (e) => {
        ...
        console.log(this.newDsForm.controls['pub'].value);
        this._hub.uploadDataset( this._hub.getId(), this._hub.getSecret(), this.newDsForm.controls['name'].value, this.newDsForm.controls['pub'].value, base64 ).subscribe(
          res => {
            const result = <HubResult> res;
            if( result.error ){
  ...
  deleteDataset( dsId: number ){
    ...
    this._hub.deleteDataset( this._hub.getId(), this._hub.getSecret(), dsId ).subscribe(
      res => {
        const result = <HubResult> res;
