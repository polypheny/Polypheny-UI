...
import {AllocationEntityModel, AllocationPartitionModel, AllocationPlacementModel, ConstraintModel, DeployMode, EntityModel, EntityType, NamespaceModel, TableModel} from '../../../models/catalog.model';
...
export class EditColumnsComponent implements OnInit, OnDestroy {
  ...
  subscribeIndexes() {
    effect(() => {
      ...
      this._crud.getIndexes(new EditTableRequest(namespace.id, entity.id)).pipe(map(r => r as RelationalResult)).subscribe(
          res => {
            this.indexHeaders.set(res.header.map(h => h?.name));
            this.indexDefinitions.set(res.data);
          }
      );
  ...
  addIndex() {
    ...
    for (const [k, v] of Object.entries(this.newIndexCols)) {
      if (v) {
        newCols.push(this.oldColumns().get(k)?.id);
      }
    }
