...
export class EditColumnsComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        effect(() => {
            ...
            this.partitioningRequest.column = colValues.length !== 0 ? oldColumns[0].name : '';
            this.newPrimaryKey = Array.from(oldColumns.values()).map(x => Object.assign({}, x));
            this.constraints.set(this._catalog.getConstraints(entity.id));
        ...
        this.partitions = computed(() => {
            ...
            return this._catalog.getPartitions(entity.id);
        });
    ...
    readonly constraints: WritableSignal<ConstraintModel[]> = signal([]);
    confirmConstraint = -1;
    ...
    readonly availableStoresForIndexes: WritableSignal<AdapterModel[]> = signal([]);
    ...
    getAvailableStoresForIndexes() {
        ...
        effect(() => {
            ...
            const stores = this._catalog.getAvailableStoresForIndexes(this.entity().id);
            if (!stores) {
                return;
            }
            this.availableStoresForIndexes.set(stores);
            if (stores?.length > 0) {
    ...
    getAddableStores(): Signal<AdapterModel[]> {
        return computed(() => {
