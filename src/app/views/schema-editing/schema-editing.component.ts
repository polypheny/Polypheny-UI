...
export class SchemaEditingComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const sub = this._crud.onReconnection().subscribe(
      b => {
        this._leftSidebar.setSchema(new SchemaRequest('/views/schema-editing/', false, 2,false, true), this._router);
      }
    );
  ...
  public getSchema() {
    this._leftSidebar.setSchema(new SchemaRequest('/views/schema-editing/', false, 2, false, true), this._router);
    this._crud.getSchema(new SchemaRequest('/views/schema-editing/', false, 2, false, true)).subscribe(
      res => {
