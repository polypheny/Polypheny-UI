...
export class DocumentEditCollectionsComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this._render.listen('document', 'click', (e: Event) => {
            ...
            if (this.editOpen && !this.inputGroup.get(0).nativeElement.contains(e.target)) {
                this.collections.update(collections => collections.map(t => {
                    t.editing = false;
                    return t;
                }));
                this.editOpen = false;
        ...
        effect(() => {
            ...
            this.collections.set(collections.map(c => Collection.fromModel(<CollectionModel>c)).sort((a, b) => a.name.localeCompare(b.name)));
        });
    ...
    readonly collections: WritableSignal<Collection[]> = signal([]);
    ...
    sendRequest(action: Method, collection: Collection) {
        ...
        const request = new QueryRequest(query, false, true, 'mql', this.namespace().id);
    ...
    createCollection() {
        ...
        this._crud.anyQueryBlocking(new QueryRequest(query, false, true, 'mql', this.namespace().id)).subscribe({
            next: (result: Result<any, any>) => {
                if (result.error) {
